...
class User_model extends CI_Model{
	...
	public function current_user(){
		...
		return $this->get_user_by_id($this->session->userdata('id'));
	}
	...
	function change_email($id, $email) {
		$this->db->where('id', $id);
		$query = $this->db->update('users', array('email' => $email));
		if($query < 1)
			return false;
		$this->db->where('id', $id);
		$query = $this->db->update('user_auth', array('email' => $email));
		if($query < 1)
			return false;
		return true;
	}
	...
	function change_user_data($id, $user_data) {
		$this->db->where('id', $id);
